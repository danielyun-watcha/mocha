...
def _top_genres(df: pd.DataFrame, domain: str, n: int = 10) -> list[dict]:
    if domain not in GENRE_DOMAINS or df.empty or "content" not in df.columns:
        return []
    gm = _load_genre_map()
    # group events by content, then sum by genre
    by_content = df.groupby("content", observed=True).agg(
        events=("user_id", "size"),
        users=("user_id", "nunique"),
    )
    by_content["genre"] = by_content.index.astype(str).map(gm).fillna("미분류")
    g = by_content.groupby("genre").agg(
        events=("events", "sum"),
        users=("users", "sum"),  # NOTE: user double-count across genres
...
def _adult_top_revenue_contents(df: pd.DataFrame, domain: str, n: int = 10) -> list[dict]:
    ...
    if purch.empty:
        return []
    def _row_price(row):
        act = str(row["action_type"])
        cid = str(row["content"]).split(":")[-1]
        m = rental_map if act == "RENTAL" else poss_map
        try:
            return int(m.get(int(cid)) or m.get(cid) or 0)
        except (ValueError, TypeError):
            return 0
    purch["price"] = purch.apply(_row_price, axis=1)
...
def _adult_revenue(df: pd.DataFrame, domain: str) -> dict:
    ...
    def _row_price(row):
        act = str(row["action_type"])
        cid_part = str(row["content"]).split(":")[-1]
        pmap = rental_map if act == "RENTAL" else (poss_map if act == "POSSESSION" else None)
        if pmap is None:
            return 0
        try:
            return int(pmap.get(int(cid_part)) or pmap.get(cid_part) or 0)
        except (ValueError, TypeError):
            return 0
...
def _adult_meta_top(df: pd.DataFrame, domain: str, kind: str, n: int = 10) -> list[dict]:
    ...
    if not cid_ints:
        return []
    # matrix 의 row = cid index. 우리 cid가 row index 와 일치한다고 가정 (실측 필요).
    import numpy as np
...
def _kpi_series(df: pd.DataFrame, domain: str, overall: list[dict]) -> tuple[list[str], dict[str, list[float]]]:
    ...
    # Pre-split df by date for content nunique / rating mean per day
    daily_df_groups = dict(list(df.groupby("date", observed=True, sort=False)))

    for d in dates:
        sub_ua = daily_events.xs(d, level="date")
        sub_df = daily_df_groups.get(d, df.iloc[0:0])
        if len(sub_ua) == 0:
