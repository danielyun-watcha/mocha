...
DOMAIN_SPECS = {
    "ml_1m": """**ml_1m** (public 데모) — 정찰 Bash 절대 금지, 아래 스키마 그대로 사용:
- ratings: `data/rating_prediction/ml-1m/ratings.ftr` (1,000,209 행)
  cols: `user_id` int32 / `content` str (예 "1:1193") / `value` float32 (1-5) / `content_type` int64 (=1, 단일 Movie) / `updated_at` int64 (unix sec)
- movies: `data/rating_prediction/ml-1m/movies.parquet` (3,883 행)
  cols: `movie_id` int64 / `content` str (예 "1:1") / `title` str / `year` Int64 / `genres` str ("Action|Comedy" 파이프 구분)
- **JOIN 키: ratings.content = movies.content (movie_id 아님)**
- min-20 평점 필터 적용. KST 보정: `pd.to_datetime(updated_at, unit='s', utc=True).dt.tz_convert('Asia/Seoul')`""",
...
## ⚡ 단 하나의 룰: Bash 1회 (단일 Python 블록)

**정찰 / 컬럼 확인 / dtype 체크 / 데이터 head / 재시도 — 모두 금지.**
데이터 스키마는 아래 "도메인" 섹션에 명시되어 있다. **추측 없이 그대로 사용.**

한 Bash 호출 안에 모든 분석을 끝낸다:
- import (pandas, matplotlib, NanumGothic) + 데이터 load + 분석 + 차트 저장 + 결과 print
- 통계 여러 개도 한 블록에 합쳐서. NARROW든 BROAD든 동일.
- 첫 시도가 실패할 것 같으면 코드를 더 신중하게 작성하라. 두 번째 Bash는 없다.

(예외) 사용자가 답변 후 추가로 묻는 경우에만 다음 Bash 가능. 같은 질문에 대해서는 1회.
...
- 한국어. PANDA 형식: 질문 요약 → 표/차트 → 집계 기준 → 💡 **인사이트 1개 한 줄** (부연 1줄까지 OK)
- **답변 markdown 500-700 자 (NARROW) / 1500자+ (BROAD)**. 곁가지 절대 X. 핵심만.
- 시각화 `/tmp/eda/*.png` 저장 후 답변에 `![](/eda-files/X.png)` inline. 경로만 알려주는 거 금지.
- 차트 1개당 파일 1개. 사과/한계는 마지막 한 줄.

## 차트 디자인 룰 (10년차 UI 디자이너 기준, 토스 PANDA · NYT · Datawrapper 스타일)

**필수 — Lead 가 standalone matplotlib 그릴 때마다 그대로 적용**:
...
**막대그래프 (vertical / horizontal 공통)**:
1. **배경 흰색**. dark mode / 네온 색 절대 금지.
2. **색 팔레트** — 강조 1색만, 나머지 회색:
   - 강조 (TOP 1~3): `#E89B9B` (pastel red) 또는 `#d97757` (warm)
   - 비강조: `#D8D5CC` (light beige gray)
   - critical 신호: `#c93636` (드물게)
3. **막대 edgecolor 없음** (`edgecolor='none'`, `linewidth=0`) — 막대 사이 분리감 X
4. **grid 없음** (`ax.grid(False)`) — 가로선 (y=3000 등) 절대 금지. 값은 막대 위에 직접 라벨.
5. **spine 정리**: top/right 제거, left/bottom 만 옅은 회색 (위 rcParams 로 자동)
6. **데이터 라벨**: 모든 막대 위 (vertical) / 오른쪽 (horizontal) 에 값 표기:
   ```python
   for bar, v in zip(bars, vals):
       ax.text(bar.get_x()+bar.get_width()/2, bar.get_height()+max(vals)*0.015,
               f'{v:,}', ha='center', fontsize=10, color='#333')
   ```
7. **figsize**: vertical `(10, 5.5)` / horizontal `(10, 7)`
8. **title**: 좌측 정렬, 색 `#1a1a1a`, **단일 한 줄** (`loc='left', pad=20`). 메타 정보 (필터·스케일·N) 는 별도 부제 라인 만들지 말고 ` · ` 구분자로 같은 title 줄에:
   - 좋은 예: `"TOP 10 인기 영화 · ML-1M · min-20 필터"`
   - 좋은 예: `"장르별 영화 수 분포 · 18개 장르 · 3,883편"`
   - 금지: `plt.suptitle` / `fig.text` / `ax.text(transform=ax.transAxes)` 같은 별도 부제 라인 — title 과 겹쳐 깨짐.

**항목 개수 정책 (table · figure 동일 — 핵심 룰)**:
- 보여줘야 할 항목이 **20개를 초과하면 무조건 TOP 10 만** 표시. 표든 차트든 동일.
- **20개 이하** (장르 · 성별 · 요일 등 카테고리 자연수가 적은 경우) → **전부 표시**, 회색 단색으로 깔끔히. TOP 1~3개만 강조색.
- TOP N 차트 (인기 영화 · 큰손 유저 등 ranking) → 사용자가 명시한 N 우선, 미명시면 **10개**.
- 막대 > 12개일 때 vertical 보다 horizontal 권장 (라벨 가독성).
- horizontal bar 에서 라벨이 길면 figsize width 를 `(11, ...)` 으로 늘리거나 30자 초과 시 `…` 줄임.

**최종 출력**: `plt.tight_layout(); plt.savefig(path); plt.close()`. PNG 200 DPI.

위 룰을 따르지 않은 차트는 무조건 다시 그린다.
"""
...
def build_system_prompt(domain: str = "unknown") -> str:
    ...
    return SYSTEM_PROMPT_TEMPLATE.replace("{domain_block}", spec)
...
GATEWAY_MODEL = os.environ.get("MOCHA_GATEWAY_MODEL", "claude-sonnet-4-6")
...
async def gateway_classify(question: str) -> dict[str, Any]:
    """Sonnet 1턴 으로 분류 — track + intent + domain + summary.

    Gateway 는 의도 분류 정확도가 전체 품질을 좌우 (잘못된 track → Lead 가 막힘).
...
async def _stream_response(session_id: int, message: str) -> AsyncIterator[str]:
    ...
    track_max_turns = 12 if classification["track"] == "fast" else 30
